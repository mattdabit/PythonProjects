...
def take_turn(num_rolls, opponent_score, dice=six_sided_dice, who='Boss Hogg'):
    ...
    dice:    dice        A function of no args and returns an integer outcome.
    who:             Name of the current player, for commentary.
    """
    assert type(num_rolls) == int, 'num_rolls must be an integer.'
...
def play(strategy0, strategy1):
    ...
    while True:
        ...
        if name(who) == 'Player 0':
            dice_amount = strategy0(score, opponent_score)
            max_allowed = num_allowed_dice(score, opponent_score)
            if dice_amount <= max_allowed:
                score += take_turn(dice_amount, opponent_score, dice, who)
            else: 
                score += take_turn(max_allowed, opponent_score, dice, who)
        elif name(who) == 'Player 1':
            dice_amount = strategy1(opponent_score, score)
            max_allowed = num_allowed_dice(score, opponent_score)
            if dice_amount <= max_allowed:
                opponent_score += take_turn(dice_amount, score, dice, who)
            else: 
                opponent_score += take_turn(max_allowed, score, dice, who)
        if score >= 100:
            return 0
...
def make_mean_strategy(min_points, num_rolls=5):
    def mean_strategy(score, opponent_score):
        bacon_value = take_turn(0, opponent_score, select_dice(score, opponent_score))
        if bacon_value > min_points and ((score + opponent_score + bacon_value) % 7 == 0 or (score + opponent_score + bacon_value) % 10 == 7):
            return 0
        else:
            return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    min_points = 4 #min_points for mean_strategy
    comeback = (opponent_score - score) >= 8
    big_comeback = (opponent_score - score) >= 16
    base_rolls = 5
    sum_score = score + opponent_score
    
    free_bacon = (opponent_score//10) + 1
    currentHogTied = sum_score %  10 == 7
    currentHogWild = sum_score % 7 == 0
    futureHogTied = (sum_score + free_bacon) % 10 == 7
    futureHogWild = (sum_score + free_bacon) % 7 == 0
    
    if (score + free_bacon >= 100):
    	return 0
    elif (free_bacon > min_points and (futureHogTied or futureHogWild or currentHogTied or currentHogWild)):
    	return 0
    elif big_comeback:
    	return base_rolls + 2
    elif comeback:
