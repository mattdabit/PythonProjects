...
def adjoin_set2(s, v):
    ...
    if set_contains2(s, v) :
        ...
        return Rlist(v, Rlist(s.first))
    else:
        return(s.first, adjoin_set2(s.rest, v))
...
def partial_tree(s, n):
    ...
    left_side = Rlist.empty
    right_side = Rlist.empty
    i = 0
    while i < left_size:
	    left_side = extend_rlist(left_side, Rlist(s.first))
	    s = s.rest
	    i += 1
    root = s.first
    s = s.rest
    k = 0
    while right_size > k:
	    right_side = extend_rlist(right_side, Rlist(s.first))
	    s = s.rest
	    k += 1
    return Tree(root, partial_tree(left_side, left_size)[0], partial_tree(right_side, right_size)[0]), s
