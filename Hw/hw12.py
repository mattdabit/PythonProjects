...
class Mobile(object):
    ...
    def __init__(self, left, right):
        self.left = left
        self.right = right
        if type(left) != Branch or type(right) != Branch:
    ...
    def isbalanced(self):
        ...
        elif isinstance(self.left.contents, Mobile) and isinstance(self.right.contents, Weight):
            return self.left.contents.isbalanced and self.left.torque == self.right.torque
        elif isinstance(self.left.contents, Weight) and isinstance(self.right.contents, Mobile):
            return self.right.contents.isbalanced and self.left.torque == self.right.torque
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        subs = s[:next_T] + t + s[next_T + 1:]
        interprets = interpret_mobile(subs)
        if interprets:
            return interpret_mobile(subs)
    return None
...
def iterator_to_stream(iterator):
    ...
    def rest():
        try:
            return Stream(next(iterator), rest)
        except StopIteration:
            return Rlist.empty
    return Stream(first, rest)
...
def make_s():
    ...
    def rest():
        ...
        factored = merge(two, three)
        return merge(factored, five)
    s = Stream(1, rest)
    return s
