...
def apply_primitive(procedure, args, env):
    ...
    def helper(procedure, args, env):
        ...
        if len(args) == 0:
            ...
            return helper(procedure, args.second, env)
    return helper(procedure, args, env)
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("too many or too few arguments given")
        else:
            bound_dict = {}
            for i in range(len(formals)):
                bound_dict[formals[i]] = vals[i]
        frame.bindings = bound_dict
        return frame
...
def do_lambda_form(vals, env):
    ...
    if len(vals.second) > 1:
        rev_body = (nil)
        for x in range(0, len(vals.second)):
            rev_body = Pair(vals.second[x], rev_body)
        body = (nil)
        for x in range(0, len(rev_body)):
            body = Pair(rev_body[x], body)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        targ = Pair(target.second, vals.second)
        targ = do_lambda_form(targ, env)
        env.define(target.first, targ)
        return target.first
    else:
...
def do_let_form(vals, env):
    ...
    for name, value in bindings:
        names, values = Pair(name, names), Pair(scheme_eval(value, env), values)
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if clause.first == "else":
            ...
            # consider error check of expression
        if scheme_true(test):
...
def do_begin_form(vals, env):
    ...
    def helper(vals, env):
        ...
        if i == stop:
            ...
            i += 1
            return helper(vals.second, env)
    return helper(vals, env)
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            procedure, args = scheme_eval(first, env), rest.map(lambda vari: scheme_eval(vari, env))
            if isinstance(procedure, PrimitiveProcedure):
                return apply_primitive(procedure, args, env)
            elif isinstance(procedure, LambdaProcedure):
